...
import ddit.vo.ChatMessageVO;
import ddit.vo.MultiChatVO;



@ServerEndpoint("/wsM.do")
public class WebSocketController {
	
	//유저 집합 리스트
	//static List<DditChatVO> sessionUsers = Collections.synchronizedList(new ArrayList<DditChatVO>());
	private static Map<String, List<MultiChatVO>> sessionUsersMap = 
			Collections.synchronizedMap(new HashMap<String, List<MultiChatVO>>());
	...
	public void handleOpen(Session userSession){
		...
		if(!sessionUsersMap.containsKey("전체")) {
			sessionUsersMap.put("전체", new ArrayList<MultiChatVO>() );
		}
	...
	public void handleMessage(String message, Session userSession) throws IOException{
		...
		if(username == null || "connect".equals(chatMessageVo.getCommand()) ){
			for(MultiChatVO chatVo : sessionUsersMap.get("전체")){
				if(userSession.equals(chatVo.getSession())){
					...
					userSession.getBasicRemote().sendText(buildJsonData("알리미", chatMessageVo.getMessage() + "님 연결 성공!!", userSession));
					
					Iterator<MultiChatVO> iterator = sessionUsersMap.get("전체").iterator();
					while(iterator.hasNext()){
						MultiChatVO chVo = iterator.next();
						if(!chVo.getSession().equals(chatVo.getSession())){
							chVo.getSession().getBasicRemote().sendText(buildJsonData("알리미", chatMessageVo.getMessage() + "님이 입장했습니다.", userSession));
						}
					}
		...
		if("create".equals(chatMessageVo.getCommand()) ){
			...
			if(sessionUsersMap.containsKey(newRoom)) {
				userSession.getBasicRemote().sendText( buildJsonData("알리미", newRoom + " 채팅방은 이미 있습니다.", null));
			}else {
				// 새로 생성된 채팅방에 저장할 List객체 생성
				List<MultiChatVO> roomMemList = new ArrayList<MultiChatVO>();
				MultiChatVO newCharVo = null;
				
				// 현재 채팅방에서 현재 Session을 갖는 DditChatVO객체를 찾는다.
				for(MultiChatVO oldChatVo : sessionUsersMap.get(room)){
				...
				if(sessionUsersMap.containsKey(room)) {
					Iterator<MultiChatVO> iterator = sessionUsersMap.get(room).iterator();
					while(iterator.hasNext()){
						MultiChatVO chVo = iterator.next();
						if(!chVo.getSession().equals(userSession)){
							chVo.getSession().getBasicRemote().sendText(buildJsonData("알리미", newCharVo.getName() + "님이 퇴장했습니다.", userSession));
						}
					}
				}
				...
				userSession.getBasicRemote().sendText(buildJsonData("알리미", newRoom + " 채팅방 생성 성공!!", userSession));
				
				// 채팅 방 목록과 해당 방의 멤버 목록을 갱신하는 메서드 호출
				roomUpdateAll(userSession);
				
				// 새로운 채팅방에서 입장하는 메시지 전송
				Iterator<MultiChatVO> iterator = sessionUsersMap.get(newRoom).iterator();
				while(iterator.hasNext()){
					MultiChatVO chVo = iterator.next();
					if(!chVo.getSession().equals(userSession)){
						chVo.getSession().getBasicRemote().sendText(buildJsonData("알리미", newCharVo.getName() + "님이 입장했습니다.", userSession));
					}
				}
			}
		}
		...
		if("change".equals(chatMessageVo.getCommand()) ){
			...
			if(room.equals(newRoom)) {
				userSession.getBasicRemote().sendText( buildJsonData("알리미", newRoom + " 채팅방은 현재 입장해있는 채팅방입니다.", null));
				return;
			}
			MultiChatVO newCharVo = null;
			...
			userSession.getBasicRemote().sendText(buildJsonData("알리미", newRoom + " 채팅방으로 이동 성공!!", userSession));
		...
		/*
		 * if("remove".equals(chatMessageVo.getCommand()) ){
		 * sessionUsersMap.remove(room); }
		 */
	}
	...
	public void roomUpdateAll(Session userSession) throws IOException{
		for(String roomName : sessionUsersMap.keySet()) {
			for(MultiChatVO chatVo : sessionUsersMap.get(roomName)) {
	...
	public void handleClose(Session userSession) throws IOException{
		System.out.println(userSession.getId() + "님이 접속을 종료하였습니다.");
		String room = (String) userSession.getUserProperties().get("room");
		
		String delName = null;
		Iterator<MultiChatVO> chatIter = sessionUsersMap.get(room).iterator();
		...
		if(sessionUsersMap.get(room).size() > 0 ) {	
			sendToAll(room, "알리미", delName + "님이 퇴장했습니다.");
		}else {
			// 채팅방에 멤버가 하나도 없으면 채팅방을 삭제한다.
			if(!"전체".equals(room)) {
				sessionUsersMap.remove(room);
	...
	public String buildJsonData(String username, String message, Session userSession){
		...
		if(userSession!=null) {
			...
			for(MultiChatVO dditCharVo : sessionUsersMap.get(room))	{
